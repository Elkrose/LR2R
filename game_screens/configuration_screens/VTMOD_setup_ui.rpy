        ...
        vbox:
            ...
            for pref in sorted(VT_Settings):
                if pref == pref_selected:
                    vbox:
                        for x in (x[0] for x in sorted(VT_Settings[pref].items(), key = lambda x: x[1][2])):
                            hbox:
                                spacing 5
                                vbox:
                                    xsize 340
                                    ysize 50
                                    yoffset 5
                                    text x style "VTtextbutton_text_style"
                                vbox:
                                    xsize 600
                                    ysize 50
                                    bar value FieldValue(persistent, VT_Settings[pref][x][0], 100, step = 1, style = "slider", action = [ Function(preference_value_changed, pref_selected) ]) xsize 600 ysize 45
                                vbox:
                                    xsize 60
                                    ysize 50
                                    yoffset 5
                                    text (str(getattr(persistent, VT_Settings[pref][x][0])) + "%" if getattr(persistent, VT_Settings[pref][x][0]) > 0 else "None") style "VTmenu_text_style" xsize 100
